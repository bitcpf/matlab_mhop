...
function [validx,validy] = regular_hexagons_circles(rad,numrads,bridgerad,hcc,melcher)
...
[tfaxvec,tfayvec] = tesselate_hexagons(maxx,maxy,tfax,tfay,pixelrad); %tesselate the whole space
[melcherxvec,melcheryvec] = tesselate_hexagons(maxx,maxy,melcherx,melchery,pixelrad);
[hccxvec,hccyvec] = tesselate_hexagons(maxx,maxy,hccx,hccy,pixelrad);
[ymcaxvec,ymcayvec] = tesselate_hexagons(maxx,maxy,ymcax,ymcay,pixelrad);
...
[validx, validy] = merge_nodes(validx,validy,pixelrad*0.5,forbidden_list); %get rid of nodes that are too close together
...
%%%%%%%%%%%%%%%%%%%%%
neighborvec = number_neighbors(validx,validy,pixelrad*1.2); %find out how many neighbors each node has
connectmat = connectivity_mat(validx,validy,pixelrad*1.3);
...
scatter(validx_1hop,validy_1hop,'ko','filled')
scatter(validx_2hop,validy_2hop,'ko','filled')
scatter(validx_3hop,validy_3hop,'ko','filled')
