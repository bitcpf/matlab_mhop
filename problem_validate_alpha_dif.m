...
vehicle_distance(index_dis)=1650;
...
temp=alpha_band(2);
alpha_band(2)=alpha_band(1);
alpha_band(1)=temp;
ideal_base=P0-10.*alpha_band(1).*log10(vehicle_distance(:)./d0);
...
for i=1:3
    ...
    newband_ss_cal=P0-10.*alpha_band(i+1).*log10(vehicle_distance(:)./d0);

    ideal_dif(i,:)=newband_ss_cal-ideal_base;
    ...
    hold on
        ...
        title([num2str(band_freq(i)/1e6),'MHz VS 5GHz'])
        xlabel('Second')
        ylabel('Signal Strength Difference')
        legend('Measured Difference','Calculated Difference')
        saveas(gcf,['validate_diff_alpha',num2str(band_freq(i)),'.jpg']);
