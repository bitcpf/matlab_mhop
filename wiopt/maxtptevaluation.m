...
used_cap=zeros(size(sum(act_link,3)));
...
for tpt_i=1:max(link_matrix(2,:))+1
   ...
   if(tpt_i-1==0)
       ...
       while(numel(lay_served)<numel(sub_matrix(1,:)))
           ...
           for path_i=1:numel(ga)
               ...
               if numel(Path)>1
                   ...
                   for pi=2:numel(Path)
                       temp=used_cap(Path(pi-1),Path(pi))+current_demand(current_node);
       ...
       if(act_cap(current_node,upper_node,current_band)>current_demand(current_node))
           ...
           act1d=ceil(tmp_idx/size(act_cap,1));
           act2d=size(act_cap,1)*act1d-tmp_idx;
           act2d(act2d==0)=size(act_cap,1);
           
           act_cap(act1d,act2d,current_band)=act_cap(act1d,act2d,current_band)-current_demand(current_node);
           ...
           act_cap(act1d,act2d,current_band)=act_cap(act1d,act2d,current_band)-act_cap(current_node,upper_node,current_band);
           tmp_idx=find(R_i(upper_node,current_node,:,:,current_band)==1);
           act1d=ceil(tmp_idx/size(act_cap,1));
           act2d=size(act_cap,1)*act1d-tmp_idx;
           act2d(act2d==0)=size(act_cap,1);
