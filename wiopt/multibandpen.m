...
for j=1:numel(mesh_nodes)
    ...
    while(path_flag==0)
        ...
        for pen_i=1:numel(p_index)
            ...
            band_store{numel(path_pen_comb)}=path_band(:);          
            end
        ...
        p_index=find(result_store(:,3)==min(result_store(:,3)));
        % Path number
        path_index=result_store(p_index,4); % map to path_store content
        temp_band=result_store(p_index,5:4+numel(bands));
    ...
    path_chosen_index=path_index(c_path_index(1))
    
    
    
    
    path_chosen=path_store{path_chosen_index}
    bands_chosen=band_store{(c_path_index)}
    ...
    for chosen_i=2:numel(path_chosen)
        ...
        R_link(path_chosen(chosen_i-1),:,bands_chosen(chosen_i-1))=0;
        R_link(:,path_chosen(chosen_i-1),bands_chosen(chosen_i-1))=0;
        R_link(path_chosen(chosen_i),:,bands_chosen(chosen_i-1))=0;
        R_link(:,path_chosen(chosen_i),bands_chosen(chosen_i-1))=0;
        
        R_link=R_link+act_link;
